...
/* Ex: Frontend provides:
 ...
 Output:
 {
     ...
     "closure_ID": 5
        ...
        const { start_date, end_date, location_ID, status, description, mnt_ID } = req.body;

        // Now enforcing description as required
        if (!start_date || !end_date || !location_ID || !status || !description || !mnt_ID) {
        ...
        const sql = `
            INSERT INTO closure (start_date, end_date, location_ID, status, description, mnt_ID)
            VALUES (?, ?, ?, ?, ?, ?)
        `;

        const values = [start_date, end_date, location_ID, status, description, mnt_ID];
